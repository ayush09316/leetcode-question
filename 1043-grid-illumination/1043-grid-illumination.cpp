...
public:
    vector<int> gridIllumination(int n, vector<vector<int>>& lamps, vector<vector<int>>& queries) {
        ...
        // 1. Check whether grid[x][y] is illuminated or not, accordingly set
        //answer either to '1' OR '0'
        // 2. if grid is illuminated, turn off the lamps at {x,y} and 8 adjacent
        //lamps = 9 lamps
        for(auto &q : queries){
            int x = q[0],y = q[1];
            if(row[x]>0 or col[y]>0 or antiDiag[x+y]>0 or mainDiag[x-y]>0){
                ...
                for(int i=-1;i<=1;i++){
                    for(int j=-1;j<=1;j++){
                        ...
                        if(nx>=0 && nx<n && ny>=0 && ny<n && s.find({nx,ny})!=s.end()){
                            row[nx]--;
                            col[ny]--;
                            antiDiag[nx+ny]--;
                            mainDiag[nx-ny]--; 
